...
class CmsAction extends Action{
	...
	* @返回赋值后的arrays 多维数组*/
	public function tags_video_read($array,$playarr=false){
		...
		//$array['keyword']       = $array['keywords'];
        $videotype = isset($channel[0]['cname_p']) ? $channel[0]['cname_p'] : $channel[0]['cname'];
        $actor = $array['actor'] ? str_replace(array('　', ' '), "{$videotype}在线观看,", $array['actor']) . "{$videotype}在线观看," : null;        
        $array['keywords']      = empty($array['keywords']) ? str_replace(array('/',' '), ',',"{$array['title']}在线观看,{$actor}{$array['area']}{$videotype}在线观看") : str_replace(',', "{$videotype}在线观看,", $array['keywords']);
        $array['vdescription']  = preg_replace('/(<.*>|\s|&nbsp;|")/U', '', mb_strimwidth($array['content'], 0, 200, "...", 'UTF-8'));//mb_substr($array['content'], 0, 150, 'UTF-8');
		...
		if($playarr){
			...
			$array['webtitle']  = "【{$array['title']}】在线观看 {$array['playname']} - {$channel[0]['cname']} - " . current(explode('-', C('web_name')));
			$array['player']    = $player;
			//显示点击数
			...
			$array['webtitle']  = "【{$array['title']}】在线观看 - {$channel[0]['cname']} - " . current(explode('-', C('web_name')));
			//显示点击数
        ...
        $array['pdescription']  = "百度影音正在为您在线播放{$channel[0]['cname']}:【{$array['title']}】 {$array['playname']}";
