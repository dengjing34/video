...
class VideoAction extends HomeAction{
    ...
    public function lists(){
		...
		$channel['count']  = $count;
		if ($url['page'] > 1) {
			$channel['webtitle'] = $channel['cname'].'视频在线观看 - 第'.$url['page'].'页 - '.current(explode('-', C('web_name')));
		}else{	
			$channel['webtitle'] = $channel['cname'].'视频在线观看 - '.current(explode('-', C('web_name')));
		}
		if($channel['pid']){
    ...
    public function search(){
		...
		$channel['pages']  = $pages;
		if ($url['page'] > 1) {
			$channel['webtitle'] = '搜索 '.$url['wd'].' 的结果列表 - 第'.$url['page'].'页-'.current(explode('-', C('web_name')));
		}else{	
			$channel['webtitle'] = '搜索 '.$url['wd'].' 的结果列表 - '.current(explode('-', C('web_name')));
		}
